...
| **Strict Mode** | Dense data analysis (logs, CSVs, massive single files). | `~/.claude/skills/rlm/rlm` |

### Phase 2: Index & Filter (The "Peeking" Phase)
**Goal**: Identify relevant data without loading it.
1.  **Native**: Use `find` or `grep -l`.
2.  **Strict**: Use `.../rlm peek "query"`.
...
1.  **Divide**: Split the work into atomic units.
    - **Strict Mode**: `.../rlm chunk --pattern "*.log"` -> Returns JSON chunks.
...
3.  **Use `rlm` binary** for programmatic slicing of large files that `grep` can't handle well.
4.  **Go is your Memory**: If you need to track state across 50 files, write a script (or use `rlm` binary) to scan them and output a summary.
...
## Recovery Mode
1.  Fall back to **Iterative Scripting**.
2.  Write a script that loads each file, runs a regex/AST check, and prints the result to stdout.
